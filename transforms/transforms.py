...
class ResizeStepScaling:
    ...
    def __call__(self, im, label=None):
        ...
        if self.count % 2 == 0:
            ...
            w = int(round(self.last_scale_factor * im.shape[1]))
            h = int(round(self.last_scale_factor * im.shape[0]))
        if self.unlabeled:
            self.count += 1
        ...
        if img_height == crop_height and img_width == crop_width:
            ...
            if crop_height > 0 and crop_width > 0:
                if self.count % 2 == 0:
                    ...
                    self.last_h_off = h_off
                    self.last_w_off = w_off
                else:
                    h_off = self.last_h_off
                    w_off = self.last_w_off
                if self.unlabeled:
                    self.count += 1
